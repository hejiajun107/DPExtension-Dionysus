    ...
    public class NCannonTargetScript : TechnoScriptable
    {
        ...
        public override void OnUpdate()
        {
            ...
            if (Inited == false)
            {
                Inited = true;
                //计算水平距离
                var technos = Finder.FindTechno(Owner.OwnerObject.Ref.Owner, t => (t.Ref.Type.Ref.Base.Base.ID == "NCANNON" || t.Ref.Type.Ref.Base.Base.ID == "NCANNOND" || t.Ref.Type.Ref.Base.Base.ID == "GYCBC" || t.Ref.Type.Ref.Base.Base.ID == "GYCCANNONBU") && t.Ref.Base.InLimbo == false && t.Ref.Base.Base.GetCoords().DistanceFrom(new CoordStruct(location.X, location.Y, t.Ref.Base.Base.GetCoords().Z)) <= 16080, FindRange.Owner);
